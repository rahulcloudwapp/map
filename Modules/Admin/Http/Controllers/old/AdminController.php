    ...
    public function isLogin()
{
    ...
    public function profile(Request $request)
    {
        ...
        if($request->submit){
             ...
             if($request->hasFile('userfile')){
              ...
              $request->userfile->move('public/img/admin', $imageName);
              $imgpath = 'public/img/admin/'.$imageName;
            }else{
            ...
            $admin->save();
            if($admin){
              return redirect('admin/profile')->with('msg','Profile updated successfully.');  
            }else{
               return redirect('admin/profile')->with('errmsg','Something went wrong.Please try after sometime');   
            }
       ...
       //echo Hash::make($request->old_pass); 
        if(Hash::check($request->old_pass,$records->password)){
           ...
           $admin->save();
           if($admin){
              return redirect('admin/profile')->with('chngmsg','Password cyhanged successfully.');  
            }else{
               return redirect('admin/profile')->with('errmsg','Something went wrong.Please try after sometime');   
            } 
        }else{
            return redirect('admin/profile')->with('errmsg','Old Password Not Match');
        }
      ...
      public function login(Request $request)
    {
        ...
        $admin = Admin::where('email',$request->email)->first();
          if($admin && Hash::check($request->password,$admin->password)){
            $sessArr = array("user_id"=>$admin->id,"username"=>$admin->name);  
            $request->session()->put('admin_sess',$sessArr);
            return redirect('admin/dashboard');
        }
        return back()->with('msg','Please check email or password.');
    }
     ...
     public function dashboard()
    {
       //  return Auth::guard('admin')->user();
        $users = User::count(); 
         $user_lot = User_lot::count();
         $auction = Auction::count();
         $category = Category::count();
        return view('admin::dashboard',compact('users','user_lot','auction','category'));
    }
